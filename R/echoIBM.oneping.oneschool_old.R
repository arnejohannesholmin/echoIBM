...
echoIBM.oneping.oneschool_old <- function(data, esnm=NULL, TVG.exp=2, compensated=c("pitch", "roll"), calibrate=TRUE, noise=c("nr", "bg", "ex"), max.memory=1e9, dumpfile="dump.txt", ask=FALSE, parlist=list(), msg=FALSE){
	...
	# If any fish are located above the sea surface, a warning is issued:
	if(any(data$pszf>0)){
		if(msg){
			warning(paste0("Some fish are located above sea level. Range of 'pszf': (", min(data$pszf), ", ", max(data$pszf), ")"))
		}
		valid <- data$pszf <= 0
		correctLength <- sapply(data, length) == length(valid) & names(data) %in% labl.TSD("ds")
	...
	cat(paste0("Transformations and rotations (number of targets: ", length(data$psxf), ") ... \n"))
