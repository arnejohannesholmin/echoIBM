...
extern "C" {
	...
	int runif_Rearr_highw_ind(int i, int j, int n)
	{
		return i + j * n;
	}
	
	// The main funciton for generating correlated vectors of autocorrelated uniform variables. Parameters to the function are:
	void runif_Rearr_highw(int *n, int *p, double *C, int *wC, int *Cind, int *w, double *prob, int *l, int *sample, int *seed, double *Z, double *U)
	{
		...
		for (int i1 = 1; i1 < *n - *w; i1++) // along beams
		{
			for (int i2 = 0; i2 < *p; i2++) // across beams
			{
				...
				int ww = 0;
				if(U[runif_Rearr_highw_ind(i1-1,i2,*n)]>0.95){
					ww = *w * 100;
				}
				else
				{
				...
				{
					cout << ww << "\n";
					cout << U[runif_Rearr_highw_ind(i1-1,i2,*n)] << "\n";
				}
				for (int i3 = 0; i3 < *w; i3++) // along the window of voxels along the current beam
				{
					for (int i4 = 0; i4 < *wC; i4++) // across beams having positive correlation to the current beam 
					{
						int thisind = Cind[ runif_Rearr_highw_ind(i2,i4,*p) ];
						if(thisind >= 0 && thisind < *p && i3<ww)
						{
							d[i3] += C[runif_Rearr_highw_ind(i2, Cind[ runif_Rearr_highw_ind(i2,i4,*p) ], *p)] * abs( U[runif_Rearr_highw_ind(i1+i3,i2,*n)] - U[runif_Rearr_highw_ind(i1-1, Cind[ runif_Rearr_highw_ind(i2,i4,*p) ], *n)] );
						}
						else
						{
							d[i3] += 1;
				...
				while(pos < *w)
				{
					...
					else if(thisrand < prob[pos])
					{
						...
						for (int i5 = 0; i5 < *l; i5++)
						{
							...
							int thissample = sample[runif_Rearr_highw_ind(i1,i5,*n)];
							if(thissample != f  &&  i1+2+thissample <= *n)
							{
								U[runif_Rearr_highw_ind(i1+posl+1,i2,*n)] = temp[thissample];
								posl += 1;
							} // End of if sample[i1* *l + k] != f
						} // End of for k
						// Insert the closest accepted value to the current position:
						U[runif_Rearr_highw_ind(i1,i2,*n)] = temp[f];
