...
extern "C" {
	...
	// Last: 2012-09-21 - Added the option of variable correlation between the beams, consistent with the need for simulating the MS70 data with high correlation between beams in the center of fans with periodic noise.
	////////////////////// DESCRIPTION: ////////////////////////
	...
	int rexp_MultSines_ind(int i, int j, int n)
	{
		return i + j * n;
	}
	
	// A modified sine function returning 0 outside the limits of the set on which the function is defined:
	double rexp_MultSines_sin0(double x, double omega, double maxx)
	{
		double out;
		if(x<0 || x>maxx)
		{
	...
	void rexp_MultSines(int *J, int *I, int *L, int *N, double *P, double *w, int *ci, double *olp, double *X, int *wM, int *seed)
	{
		...
		*P = *P * 2 * PI;
		...
		for(int l = 0; l < W * *L; l++)
		{
			...
			{
				add = floor(l / *L) - W + 1;
				phis[rexp_MultSines_ind(l, i, W * *L)] = rand() / double(RAND_MAX) + add;
			}
		}
		...
		for(int n = 0; n < *N; n++)
		{
			...
			{
				s[p] = rexp_MultSines_sin0(npos - phis[p], *P, *w);
			}
			...
			for(int i = 0; i < *I; i++)
			{
				...
				for(int ii = 0; ii < 2 * *ci + 1; ii++)
				{
					// Only add the contribution from the targets in the beam if the value of 'olp' is positive:
					if(olp[rexp_MultSines_ind(ii, i, ncor)]>0)
					{
						// Loop through the 'WL' relevant targets:
						for(int l = 0; l < WL; l++)
						{
							S[rexp_MultSines_ind(n, i, *N)] += s[rexp_MultSines_ind(l, i+ii, WL)] * olp[rexp_MultSines_ind(ii, i, ncor)];
						}
					}
				}
			}
		...
		for(int i = 0; i < *I; i++)
		{
			...
			for(int n = 1; n < *N - 1; n++)
			{
				...
				if( S[rexp_MultSines_ind(n-1, i, *N)]<S[rexp_MultSines_ind(n, i, *N)] && S[rexp_MultSines_ind(n, i, *N)]>S[rexp_MultSines_ind(n+1, i, *N)] )
				{
					...
					if(n-lastMaxPos > *wM)
					{
						...
						if(abs(n-midP) < abs(MaxPosClosest-midP))
						{
							...
							X[rexp_MultSines_ind(j, i, *J)] = pow(S[rexp_MultSines_ind(n, i, *N)],2);
					...
					else if(S[rexp_MultSines_ind(n, i, *N)] > lastMax)
					{
						...
						{
							X[rexp_MultSines_ind(j, i, *J)] = pow(S[rexp_MultSines_ind(lastMaxPosClosest, i, *N)],2);
							break;
						}
					}
				} // End of if
		...
		for(int j = 1; j < *J; j++)
		{
			...
			for(int l = 0; l < (W-1) * *L; l++)
			{
				for(int i = 0; i < fullI; i++)
				{
					phis[rexp_MultSines_ind(l, i, W * *L)] = phis[rexp_MultSines_ind(l + *L, i, W * *L)];
				}
			}
			for(int l = (W-1) * *L; l < W * *L; l++)
			{
				for(int i = 0; i < fullI; i++)
				{
					phis[rexp_MultSines_ind(l, i, W * *L)] = rand() / double(RAND_MAX) + j;
				}
			}
